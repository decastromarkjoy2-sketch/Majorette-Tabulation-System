...
export function createJudgesRouter(database: typeof db = db): IRouter {
  ...
  router.post("/judges", async (req, res): Promise<void> => {
    ...
    let result:
      | {
          status: "created";
          judge: typeof judgesTable.$inferSelect;
          accessCode: string;
        }
      | { status: "duplicate" }
      | { status: "full" };
    ...
    try {
      result = await database.transaction(async (tx) => {
        await tx.execute(
          sql`select pg_advisory_xact_lock(${JUDGE_ROSTER_LOCK_ID})`,
        );
      ...
      if (isUniqueViolation(error)) {
        res
          .status(409)
          .json({ error: "A judge with this name is already registered." });
        return;
      }
      throw error;
    }
  ...
  router.post("/judges/:id/access-code", async (req, res): Promise<void> => {
    ...
    const params = ResetJudgeAccessCodeParams.safeParse({
      id: parseInt(raw, 10),
    });
    if (!params.success) {
    ...
    res.json(
      ResetJudgeAccessCodeResponse.parse({
        ...judge,
        hasAccessCode: true,
        accessCode,
