...
export function createScoresRouter(database: typeof db = db): IRouter {
  ...
  router.post("/scores", async (req, res): Promise<void> => {
    ...
    if (session?.role !== "judge" || !session.judgeId) {
      res
        ...
        .json({
          error:
            "Sign in with your judge access code before submitting scores.",
        });
      return;
    }
    ...
    const judge = judges.find(
      (registeredJudge) => registeredJudge.id === judgeId,
    );
    if (!judge) {
    ...
    if (judge.accessCodeVersion !== session.accessCodeVersion) {
      res.status(401).json({
        error:
          "Your judge session is no longer valid. Sign in again with the current access code.",
      });
      return;
    }
    ...
    if (
      existingScore &&
      isDuplicateScoreSubmission([existingScore], {
        judgeId,
        category,
        schoolCode,
      })
...
export default createScoresRouter();
