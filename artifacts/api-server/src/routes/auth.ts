...
router.post("/auth/organizer-sessions", (req, res): void => {
  ...
  if (!process.env.ORGANIZER_ACCESS_CODE) {
    ...
    res.status(503).json({ error: "Organizer access is not configured. Contact the system owner." });
    return;
  }
  if (!verifyOrganizerCode(parsed.data.accessCode)) {
    res.status(401).json({ error: "That organizer access code is not valid." });
    return;
  }

  startSession(res, "organizer");
  res.status(201).json(GetSessionResponse.parse({ role: "organizer", judgeId: null }));
});

router.post("/auth/judge-sessions", async (req, res): Promise<void> => {
  const parsed = CreateJudgeSessionBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: "Select your name and enter your access code." });
    return;
  }
  ...
  if (!judge?.accessCodeHash || !verifyAccessCode(parsed.data.accessCode, judge.accessCodeHash)) {
    res.status(401).json({ error: "The judge name or access code is not valid." });
    return;
  }

  startSession(res, "judge", judge.id, judge.accessCodeVersion);
  res.status(201).json(GetSessionResponse.parse({ role: "judge", judgeId: judge.id }));
});
