import { useGetGroupTabulation } from "@workspace/api-client-react";
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4 text-sm text-white/60 bg-black/40 px-4 py-2 rounded-lg border border-white/5">
          <span className="flex items-center gap-2"><Users className="h-4 w-4" /> {tabulation.totalJudges} Judges Active</span>
                ...
                {sortedEntries.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={9} className="h-32 text-center text-muted-foreground">
                      No scores submitted yet for Group Category.
                    </TableCell>
                  </TableRow>
                ) : (
                  sortedEntries.map((entry, idx) => (
                    <TableRow 
                      ...
                      className={`border-white/5 group transition-colors hover:bg-white/5 ${idx === 0 ? 'bg-primary/5' : ''}`}
                    >
                      ...
                      <TableCell className="text-right font-mono font-black text-2xl text-primary tracking-tighter">
